// code taken from cava: https://github.com/karlstav/cava
...
void *input_alsa(void *data) {
    ...
    frames = 8192;//frames = period_size / ((audio->format / 8) * CHANNELS_COUNT);
    // printf("period size: %lu\n", period_size);
    // exit(0);

    // frames * bits/8 * channels
    // const int size = frames * (audio->format / 8) * CHANNELS_COUNT;
    signed char *buffer = (signed char*) malloc(period_size);
